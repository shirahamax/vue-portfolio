    ...
    <v-toolbar flat height="35px" class="white item-toolbar">
      <v-tabs centered show-arrows hide-slider class="white">
        <v-tab
          ...
          :key="category.id"
          @click="sortItems(category.id)"
        >
          <span
            class="caption"
            :class="
              currentCatId == category.id ? 'red--text font-weight-bold' : ''
            "
          >
            {{ category.name }}</span
          >
          ...
          <nuxt-link :to="{ name: 'items-id', params: { id: item.id } }">
            <v-card flat class="black mt-1" style="margin:0 auto;">
              ...
              <v-img :src="item.image">
                <v-layout v-if="item.is_sold_out" justify-start>
...
export default {
  ...
  computed: {
    ...mapState("itemModule", ["categories", "currentCatId"]),
    ...mapGetters("itemModule", ["sortedItemsByCat"]),
    breakpoint() {
      return this.$vuetify.breakpoint;
    }
  },
  fetch({ store }) {
    return store.dispatch("itemModule/getCategories");
  },
  created() {
    // URLからcategoryIdを取得
    const catIdInUrl = this.$route.query.categoryId;
